...
Without `--inject`, `dispatch` only records the attempt: the task turns `dispatched`, the terminal is not told, and the response reports `injected: false` (newer runtimes also add a `warning` saying so). Deliver the task yourself, or reset it (`task-update --status failed`, then `--status ready`) and dispatch again with `--inject`.

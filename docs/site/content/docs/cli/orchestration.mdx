...
- `--peek` / `--all` do not consume mail and return no Delivery id, so they cannot be acknowledged. To drain a mailbox, repeat plain `check` and `--ack` each Delivery until `deliveryId` is `null`.

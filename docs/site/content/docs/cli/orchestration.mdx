...
- `--peek` / `--all` do not consume mail. Both return at most 100 rows and no Delivery id, so the rows they show cannot be acknowledged and a peek-only loop keeps reporting the same pending ones — which looks like a mailbox losing later messages. An `--ack <delivery_id>` passed alongside them still acknowledges that Delivery. To drain a mailbox, repeat plain `check` and `--ack` each Delivery until `deliveryId` is `null`.
